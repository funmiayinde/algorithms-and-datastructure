...
const isAdditiveNumber = (num: string) => {
    ...
    for (let i = 1; i < num.length / 2; i++) {
        for (let j = 1; j <= num.length / 2; j++){
            if (canBeAdded(num.substr(0, 1), num.substr(i, j), num.substr(i + j))){
                return true;
            }
        }
    }
...
const canBeAdded = (a: string, b: string, c: string): boolean => {
    if ((a.length > 1 && a.charAt(0) === '0')
     || (b.length > 1 && a.charAt(0) === '0')
     || (c.length > 1 && a.charAt(0) === '0')){
         return false;
     }
     
     let aNum: number = parseInt(a);
     let bNum: number = parseInt(b);
     let sum: string = aNum + bNum + '';
     if (c === sum) {
         return true;
     }
